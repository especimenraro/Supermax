...
function blossom_recipe_setup() {
	...
	register_nav_menus( array(
        ...
        'secondary' => esc_html__( 'Secondary', 'blossom-recipe' ),
	) );
...
function blossom_recipe_change_comment_form_default_fields( $fields ){    
    ...
    $fields['author'] = '<p class="comment-form-author"><input id="author" name="author" placeholder="' . esc_attr__( 'Name*', 'blossom-recipe' ) . '" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></p>';
    
    $fields['email'] = '<p class="comment-form-email"><input id="email" name="email" placeholder="' . esc_attr__( 'Email*', 'blossom-recipe' ) . '" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></p>';
    
    $fields['url'] = '<p class="comment-form-url"><input id="url" name="url" placeholder="' . esc_attr__( 'Website', 'blossom-recipe' ) . '" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" /></p>'; 
...
function blossom_recipe_change_comment_form_defaults( $defaults ){    
    $defaults['comment_field'] = '<p class="comment-form-comment"><textarea id="comment" name="comment" placeholder="' . esc_attr__( 'Comment', 'blossom-recipe' ) . '" cols="45" rows="8" aria-required="true"></textarea></p>';
        ...
        <div class="welcome-message notice notice-info">
            <div class="notice-wrapper">
                <div class="notice-text">
                    ...
                    <p><?php printf( __( '%1$s is now installed and ready to use. Click below to see theme documentation, plugins to install and other details to get started.', 'blossom-recipe' ), $name ) ; ?></p>
...
if( ! function_exists( 'blossom_recipe_admin_notice_display' ) ) :
/**
 * ignore notice
 */
function blossom_recipe_admin_notice_display() {

    update_option( 'blossom-recipe-update-notice', false );

}
endif;
add_action( 'after_switch_theme', 'blossom_recipe_admin_notice_display' );
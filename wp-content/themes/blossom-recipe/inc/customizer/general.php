...
function blossom_recipe_customize_register_general( $wp_customize ){
    ...
    $wp_customize->add_control(
        new Blossom_Recipe_Toggle_Control( 
            ...
            array(
                'section'     => 'post_page_settings',
                'label'       => __( 'Hide Author Section', 'blossom-recipe' ),
                'description' => __( 'Enable to hide author section.', 'blossom-recipe' ),
            )
        )
    );
    ...
    if( blossom_recipe_is_btif_activated() ){
        ...
        $wp_customize->add_control(
            new Blossom_Recipe_Note_Control( 
                ...
                array(
                    'section'     => 'instagram_settings',
                    'description' => sprintf( __( 'You can change the setting BlossomThemes Instagram Feed %1$sfrom here%2$s.', 'blossom-recipe' ), '<a href="' . esc_url( admin_url( 'admin.php?page=class-blossomthemes-instagram-feed-admin.php' ) ) . '" target="_blank">', '</a>' )
                )
            )
        );        
    }else{
        ...
        $wp_customize->add_control(
            new blossom_recipe_Plugin_Recommend_Control(
                ...
                array(
                    ...
                    'description' => sprintf( __( 'Please install and activate the recommended plugin %1$sBlossomThemes Instagram Feed%2$s. After that option related with this section will be visible.', 'blossom-recipe' ), '<strong>', '</strong>' ),
                )
            )
        );
    }

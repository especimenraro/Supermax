...
<div class="berocket_compare_widget berocket_compare_widget_<?php echo $type; ?>" data-type="<?php echo $type; ?>">
    ...
    if ( isset( $products ) && is_array( $products ) && count( $products ) > 0 ) {
        foreach ( $products as $product ) {
            ...
            echo '<li class="br_widget_compare_product_'.intval($default_product).'">';
            echo '<a href="#remove" class="br_remove_compare_product" data-id="'.intval($default_product).'"><i class="fa fa-times"></i></a>';
            echo '<a href="'.$link.'">';
            if ( @ $type != 'text' ) {
                echo $image;
            }
